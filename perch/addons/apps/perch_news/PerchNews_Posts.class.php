...
class PerchNews_Posts extends PerchAPI_Factory
{
    protected $table               = 'news_posts';
    protected $pk                  = 'postID';
    protected $singular_classname  = 'PerchNews_Post';
    ...
    */
    public function find($postID, $is_admin=false)
    {
        ...
        if ($Cache->exists('p'.$postID)) {
            ...
            $sql = 'SELECT * FROM '.PERCH_DB_PREFIX.'news_posts WHERE postID='.$this->db->pdb((int)$postID);
            ...
            // if(is_array($row)) {
            //     $sql = 'SELECT categoryID FROM '.PERCH_DB_PREFIX.'news_posts_to_categories WHERE postID='.$this->db->pdb((int)$postID);
    ...
    {
        $sql = 'SELECT * FROM '.PERCH_DB_PREFIX.'news_posts WHERE postImportID='.$this->db->pdb($importID);
        $row = $this->db->get_row($sql);
    ...
    */
    public function find_by_slug($postSlug)
    {
        ...
        if ($Cache->exists('p'.$postSlug)) {
            ...
            if (self::$preview_mode) {
                $sql = 'SELECT * FROM '.PERCH_DB_PREFIX.'news_posts WHERE postSlug='.$this->db->pdb($postSlug);
            }else{
                $sql = 'SELECT * FROM '.PERCH_DB_PREFIX.'news_posts WHERE postStatus=\'Published\' AND postDateTime<='.$this->db->pdb(date('Y-m-d H:i:00')).' AND postSlug='.$this->db->pdb($postSlug);
            }

            $row = $this->db->get_row($sql);

            // if(is_array($row)) {
            //     $sql = 'SELECT categoryID FROM '.PERCH_DB_PREFIX.'news_posts_to_categories WHERE postID = '.$this->db->pdb($row['postID']);
            //     $result = $this->db->get_rows($sql);
            //     $a = array();
            //     if(is_array($result)) {
            //         foreach($result as $cat_row) {
	...
	*/
    public function create($data, $Template=false)
    {
		...
		if ($postID) {
			...
			// 	    $this->db->insert(PERCH_DB_PREFIX.'news_posts_to_categories', $tmp);
			// 	}
			// }
    ...
    public function get_custom($opts)
    {
        ...
        $opts['template'] = function($items) use ($set_template) {
            if (isset($set_template) && $set_template!=false) {
                $template = 'news/'.str_replace('news/', '', $set_template);
            }else{
                if (PerchUtil::count($items)==1) {
                    $template = 'news/'.$items[0]->postTemplate();
                }else{
    ...
    public function get_by_section_slug_for_admin_listing($slug, $Paging)
    {
        ...
        $sql = $Paging->select_sql(). ' p.*
                FROM '.$this->table.' p, '.PERCH_DB_PREFIX.'news_sections s
    ...
    public function get_by_status($status='PUBLISHED', $sectionID=false, $newsSlug=false, $Paging=false)
    {
        $newsID = $this->_news_slug_to_id($newsSlug);
        ...
        $sql .= ' p.*
                FROM '.$this->table.' p
                WHERE p.postStatus='.$this->db->pdb($status) .' AND newsID='.$this->db->pdb((int)$newsID);
    ...
    public function get_years($sectionID=null, $newsID=null)
    {
        $Cache = PerchNews_Cache::fetch();

        $cache_key = 'get_years';
        if ($sectionID) $cache_key.='_s'.$sectionID;
        if ($newsID) $cache_key.='_b'.$newsID;
        ...
        if ($newsID) $sql .= ' AND newsID='.$this->db->pdb($newsID);
    ...
    public function update_category_counts()
    {
        ...
        $sql = 'DELETE FROM '.PERCH_DB_PREFIX.'category_counts WHERE countType='.$this->db->pdb('news.post');
        $this->db->execute($sql);
        ...
        if (PerchUtil::count($rows)) {
            ...
            foreach($rows as $row) {
                ...
                if ($Category) {
                    $Category->update_count('news.post', $row['qty']);
                }
            }
        }
    }
    ...
    private function _standard_where_callback($opts)
    {
        ...
        return function(PerchQuery $Query) use ($opts, $preview_mode, $db) {
            ...
            // news
            if (isset($opts['news'])) {
                $newsID = $this->_news_slug_to_id($opts['news']);
                $Query->where[] = ' newsID='.(int)$newsID.' ';
            }
            ...
            if (isset($opts['tag'])) {
                ...
                if (is_array($tags)) {
                    ...
                    $Query->select .= ' LEFT JOIN '.PERCH_DB_PREFIX.'news_posts_to_tags p2t ON tbl.postID=p2t.postID LEFT JOIN '.PERCH_DB_PREFIX.'news_tags t ON p2t.tagID=t.tagID ';
                    ...
                    if (PerchUtil::count($do_not_tag)) {
                        $Query->where[] = ' tbl.postID NOT IN (
                                SELECT p2.postID FROM '.$this->table.' p2, '.PERCH_DB_PREFIX.'news_posts_to_tags p2t2, '.PERCH_DB_PREFIX.'news_tags t2
    ...
    private function _news_slug_to_id($slug)
    {
        if (!$slug) return 1;

        if (PERCH_RUNWAY) {
            $sql = 'SELECT newsID FROM '.PERCH_DB_PREFIX.'newss WHERE newsSlug='.$this->db->pdb($slug).' LIMIT 1';
