<?php
    ...
    echo $HTML->para('This page lists your news posts. You can filter them by category or status.');

    if ($CurrentUser->has_priv('perch_news.import')) {
        echo $HTML->heading3('Import');
        echo $HTML->para('If moving from a different news system, you may be able to %simport your posts%s', '<a href="'.$API->app_path().'/import/">', '.</a>');
    }
    ...
    if ($CurrentUser->has_priv('perch_news.post.create')) echo '<a class="add button" href="'.$HTML->encode($API->app_path().'/edit/'.(PERCH_RUNWAY ? '?news='.$News->id() : '')).'">'.$Lang->get('Add Post').'</a>';
        ...
        <?php
            if (PerchUtil::count($newss)) {
                foreach($newss as $Item) {
                    ...
                    echo '<a href="'.PerchUtil::html($API->app_path()).'?news='.$Item->newsSlug().'">'.$Item->newsTitle().'</a></li>';
                }
            }
        ?>
        <li class="new <?php echo ($filter=='status'&&$status=='draft'?'selected':''); ?>"><a href="<?php echo PerchUtil::html($API->app_path().'?status=draft&news='.$News->newsSlug()); ?>"><?php echo $Lang->get('Drafts'); ?></a></li>
        <?php
    ...
    <table class="d">
        <thead>
            <tr>
                ...
                <?php if ($CurrentUser->has_priv('perch_news.post.delete')) { ?>
