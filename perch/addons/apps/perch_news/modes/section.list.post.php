<?php
    ...
    echo '<a class="add button" href="'.$HTML->encode($API->app_path().'/sections/edit/?news='.$News->id()).'">'.$Lang->get('Add Section').'</a>';
        ...
        <?php
            if (PerchUtil::count($newss)) {
                foreach($newss as $Item) {
                    ...
                    echo '<a href="'.PerchUtil::html($API->app_path()).'/sections/?news='.$Item->newsSlug().'">'.$Item->newsTitle().'</a></li>';
                }
            }
        ?>
    </ul>
