...
class PerchNews_Authors extends PerchAPI_Factory
{
    protected $table               = 'news_authors';
    protected $pk                  = 'authorID';
    protected $singular_classname  = 'PerchNews_Author';
    ...
    public function update_post_counts()
    {
        $sql = 'SELECT authorID, COUNT(*) AS qty
                FROM '.PERCH_DB_PREFIX.'news_posts 
        ...
        if (PerchUtil::count($rows)) {
            ...
            $sql = 'UPDATE '.PERCH_DB_PREFIX.'news_authors SET authorPostCount=0';
            $this->db->execute($sql);

            foreach($rows as $row) {
                $sql = 'UPDATE '.PERCH_DB_PREFIX.'news_authors SET authorPostCount='.$this->db->pdb($row['qty']).' WHERE authorID='.$this->db->pdb((int)$row['authorID']).' LIMIT 1';
                $this->db->execute($sql);
            }
        }
    }
