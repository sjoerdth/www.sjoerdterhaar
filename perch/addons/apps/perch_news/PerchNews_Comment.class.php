...
class PerchNews_Comment extends PerchAPI_Base
{
    ...
    public function set_status($status)
    {
        ...
        $akismet_api_key = $Settings->get('perch_news_akismet_key')->val();
        ...
        if ($akismet_api_key) {
            ...
            if ($this->commentStatus()=='SPAM' && $status=='LIVE') {
                ...
                if (PerchUtil::count($spam_data)){
                    PerchNews_Akismet::submit_ham($akismet_api_key, $spam_data['fields'], $spam_data['environment']);
                }
            }
            ...
            if ($status=='SPAM') {
                ...
                if (PerchUtil::count($spam_data)){
                    PerchNews_Akismet::submit_spam($akismet_api_key, $spam_data['fields'], $spam_data['environment']);
                }
            }
