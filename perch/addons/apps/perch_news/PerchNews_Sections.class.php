...
class PerchNews_Sections extends PerchAPI_Factory
{
    protected $table     = 'news_sections';
	protected $pk        = 'sectionID';
	protected $singular_classname = 'PerchNews_Section';

	protected $default_sort_column = 'sectionTitle';

	public $static_fields   = array('sectionTitle', 'sectionSlug', 'sectionPostCount', 'newsID');
	...
	/**
	 *
	 * retrieves all sections used by news posts along with a count of number of posts for each section.
	 */
	 public function all_in_use() {
		$sql = 'SELECT sectionID, sectionTitle, sectionSlug, sectionPostCount, sectionPostCount AS qty, sectionDynamicFields
                FROM '.PERCH_DB_PREFIX.'news_sections WHERE sectionPostCount>0
	...
	public function get_custom($opts)
	{
        ...
        if ($opts['news']) {
            ...
            if ($News) {
                $newsID = (int)$News->id();
                $newsSQL = ' AND newsID='.$this->db->pdb($newsID) .' ';
            }
        }

		if ($opts['include-empty']) {
			$sql = 'SELECT *, sectionID AS _id, sectionPostCount as qty FROM '.$this->table.' WHERE 1=1 '.$newsSQL.' ORDER BY sectionTitle ASC';
		}else{
			$sql = 'SELECT *, sectionID AS _id, sectionPostCount as qty FROM '.$this->table.' WHERE sectionPostCount>0 '.$newsSQL.' ORDER BY sectionTitle ASC';
		}
        ...
        // post process
        $tags   = $Template->find_all_tags_and_repeaters('news');
	...
	public function update_post_counts()
    {
    	$sql = 'SELECT c.sectionID, COUNT(p.postID) AS qty
                FROM '.PERCH_DB_PREFIX.'news_sections c, '.PERCH_DB_PREFIX.'news_posts p
        ...
        if (PerchUtil::count($rows)) {
            ...
            $sql = 'UPDATE '.PERCH_DB_PREFIX.'news_sections SET sectionPostCount=0';
            $this->db->execute($sql);

        	foreach($rows as $row) {
        		$sql = 'UPDATE '.PERCH_DB_PREFIX.'news_sections SET sectionPostCount='.$this->db->pdb($row['qty']).' WHERE sectionID='.$this->db->pdb($row['sectionID']).' LIMIT 1';
        		$this->db->execute($sql);
        	}
        }
    }
