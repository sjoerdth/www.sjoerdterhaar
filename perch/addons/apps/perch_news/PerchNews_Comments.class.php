...
class PerchNews_Comments extends PerchAPI_Factory
{
	protected $table     = 'news_comments';
	protected $pk        = 'commentID';
	protected $singular_classname = 'PerchNews_Comment';
	...
	public function get_by_status($status='pending', $Paging=false)
	{
        ...
        $sql .= ' c.*, p.postTitle
                FROM ' . $this->table .' c, '.PERCH_DB_PREFIX.'news_posts p
	...
	public function get_custom($postID, $opts)
	{
	    ...
	    if (isset($opts['template'])) {
	        $template = 'news/'.$opts['template'];
	    }else{
	        $template = 'news/comment.html';
	    }
	...
	public function receive_new_comment($SubmittedForm)
	{
		...
		if ($input['postID']) {
			...
			if (is_object($Post)) {
				...
				$akismetAPIKey = $Settings->get('perch_news_akismet_key')->val();
	...
	private function _notify_author_of_comment($Post, $Comment)
	{
		...
		if ($Settings->get('perch_news_comment_notify')->val()=='1') {

			$msg = $Comment->commentName().' ('.$Comment->commentEmail().'):'."\n\n";
			$msg .= strip_tags($Comment->commentHTML())."\n\n";
			$msg .= 'http://'.$_SERVER['SERVER_NAME'].PERCH_LOGINPATH.'/addons/apps/perch_news/comments/edit/?id='.$Comment->id()."\n\n";
	...
	private function _check_for_spam($fields, $environment, $akismetAPIKey=false)
    {
    	...
    	if ($akismetAPIKey) {
	    	if (!class_exists('PerchNews_Akismet')) {
	    		include_once('PerchNews_Akismet.class.php');
	    	}
	        if (PerchNews_Akismet::check_message_is_spam($akismetAPIKey, $fields, $environment)) {
