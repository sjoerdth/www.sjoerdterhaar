...
class PerchNews_Util extends PerchAPI_Factory
{
	...
	{
		$app_templates = $this->get_dir_contents(__DIR__.'/templates/news');
        $local_templates = $this->get_dir_contents(PERCH_TEMPLATE_PATH.'/news');
	...
	public function import_from_wp($wordpress_file, $format="textile", $callback=false, $sectionID=1)
	{
	    ...
	    // AUTHORS
	    $Authors = new PerchNews_Authors($this->api);
	    ...
	    // POSTS
	    $Posts   = new PerchNews_Posts($this->api);

		$Template = $this->api->get('Template');
		$Template->set('news/post.html', 'news');
	    ...
	    foreach($xml->channel->item as $item) {
	        ...
	        if (is_object($Post)) {
	            ...
	            $Tags = new PerchNews_Tags($this->api);
	            ...
	            foreach($item->category as $category) {
	                ...
	                switch((string)$attributes['domain']) {
	                    ...
	                    case 'category':
	                        ...
	                        $Category = $Categories->find_or_create('news/'.$slug.'/', $label);
	            ...
	            // COMMENTS
	            $Comments = new PerchNews_Comments($this->api);
	            foreach($item->children('wp', true) as $tag) {
	                $tagName = $tag->getName();
